...
export const preloadVATAssets = (metaUrl: string): Promise<VATData> => {
  ...
  const promise = (async () => {
    try {
      // Use fetch directly so we can inspect the response before parsing
      const response = await fetch(metaUrl)
      if (!response.ok) {
        throw new Error(`VAT meta fetch failed: HTTP ${response.status} for ${metaUrl}`)
      }
      const text = await response.text()
      // Guard against IDM / server intercepting the request and returning HTML
      const trimmed = text.trimStart()
      if (trimmed.startsWith('<') || trimmed.startsWith('<!')) {
        throw new Error(
          `VAT meta at "${metaUrl}" returned HTML instead of JSON. ` +
          `Check that the file exists in /public/vat/ and no browser extension (e.g. IDM) is intercepting the request.`
        )
      }
      const meta = JSON.parse(text) as VATMeta
