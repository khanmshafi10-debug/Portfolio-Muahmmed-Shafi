...
export function Stars({
  count = 350,   // was 200 (perf) / was 500 (original) — 350 is atmospheric
  radius = 190,
  speed = 2,
  axis = [0.2, 1, 0],
}: StarsProps) {
  ...
  const uniforms = useMemo(() => ({
    uScale: uniform(0.4),
    uColor: uniform(new THREE.Color('#e0ecff')),
    uHueVar: uniform(0.25),
    uRim: uniform(0.95),
    uSpeed: uniform(2.5),
  ...
  // Throttle star rotation — sky barely moves, update every 2nd frame
  const starFrame = useRef(0);
  useFrame(({ clock }) => {
    starFrame.current++;
    if (starFrame.current % 2 !== 0) return;
    if (groupRef.current) {
