...
export function useCharacterPhysics(
  ...
  useFrame((_, delta) => {
    ...
    const targetWeights = calculateBlendWeights(
      // FPV (tank): speed<0 = forward; cam mode: speed>0 = forward
      // calculateBlendWeights expects +speed = forward, so negate only in FPV
      cameraMode === CameraMode.FPV ? -s.speed : s.speed,
      isRotating,
      s.walkSpeed,
      s.runSpeed,
      s.backSpeed
