...
export const solveTank = (
  ...
  // In Three.js local space, -Z is the forward direction.
  // translateZ(-) = move forward, translateZ(+) = move backward
  let targetSpeed = 0;

  if (moveForward) {
    targetSpeed = run ? -s.runSpeed : -s.walkSpeed; // negative = forward
  } else if (moveBackward) {
    targetSpeed = s.backSpeed;                       // positive = backward
  }
