...
export const DEFAULT_BLADES_PER_AXIS = 768;   // 44% fewer than original 1024, 50% more than our 512 — best balance
export const DEFAULT_GRASS_AREA_SIZE = 70;    // between original 80 and our 64
/** Number of blade spacings per grid snap step. gridCellSize = BLADE_SPACING * this. */
export const DEFAULT_BLADE_STEPS_PER_CELL = 1;

// Default LOD segments — quality/performance balanced
export const DEFAULT_LOD_SEGMENTS_CONFIG = [
  {
    segments: 12,         // was 8 (perf) / was 15 (original) — 12 is visually rich up close
    minDistance: 0,
    maxDistance: 5,
  ...
  },
  {
    segments: 4,
    minDistance: 5,
    maxDistance: 18,      // extended from 15 for smoother LOD transition
