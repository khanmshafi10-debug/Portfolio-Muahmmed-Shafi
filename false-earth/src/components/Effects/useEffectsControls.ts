...
export function useEffectsControls() {
  ...
  const smaaParams = useControls('Effects.SMAA', {
    enabled: { value: true, label: 'Enable SMAA' }, // ON — critical for HD sharp edges
  }, { collapsed: true });
  ...
  const dofParamsTPS = useControls('Effects.DoF.TPS', {
    enabled: { value: true, label: 'Enable Depth of Field' }, // ON — cinematic depth
    ...
    focalLength: { value: 28.0, min: 0.01, max: 100, step: 0.1 },
    bokehScale: { value: 3.5, min: 0.0, max: 10.0, step: 0.1 }, // was 5 — more subtle
  }, { collapsed: true });

  const dofParamsFREE = useControls('Effects.DoF.FREE', {
    enabled: { value: false, label: 'Enable Depth of Field' }, // disabled by default
